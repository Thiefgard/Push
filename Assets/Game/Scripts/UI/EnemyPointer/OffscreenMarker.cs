...
class OffscreenMarkersCameraScript : MonoBehaviour {
    ...
    public void Remove(OffscreenMarker om)
    {
        if (_trackedObjects.Contains(om))
        {
            _trackedObjects.Remove(om);
        }
        else
        {
            //Debug.LogWarning("EntTrackerLib: The tracked objects list doesn't contain " + om, om);
        }
    }
    ...
    void OnGUI() {
        ...
        for ( int i = _trackedObjects.Count - 1; i >= 0; i-- ) {
            ...
            if ( marker && marker.gameObject && marker.IsActivate) {
