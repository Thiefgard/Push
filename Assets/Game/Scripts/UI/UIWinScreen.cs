...
public class UIWinScreen : UIElement
{
    ...
    [SerializeField] private GameObject _btnAds;
    ...
    public void OnAddFinishCoinReward()
    {
        if (Services.Instance.FadsService is { HasRewardedVideo: true })
        {
            Services.Instance.FadsService.RewardedShouldReward += AddCoinFinishReward;
            Services.Instance.FadsService.ShowRewardedVideo(Placements.PLACEMENT_REWARDED_LEVEL_GETHINTS);
        }
    }

    private void AddCoinFinishReward()
    {
        Services.Instance.FadsService.RewardedShouldReward -= AddCoinFinishReward;
        GameManager.Instance.AddCoins(250);
    ...
    private void FixedUpdate()
    {
        if(_isCoinsFly)
        {
            ...
            if(_listCoins.Count == 0)
            {
                ...
                if (!_isWatchAds) _btnAds.SetActive(true);
                else _btnAds.SetActive(false);

                StartCoroutine(ShowContiniue());
                //_btnCollect.GetComponent<Animator>().Play("ShowPanel", 0, 0.0f);
                _btnAds.GetComponent<Animator>().Play("ShowPanel", 0, 0.0f);
